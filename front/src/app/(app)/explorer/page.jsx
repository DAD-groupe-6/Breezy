...
export default function ExplorerPage() {
  ...
  return (
    <div className="relative min-h-full overflow-hidden bg-transparent px-4 py-6 text-[var(--color-text-primary)] md:px-6 lg:px-8">
      ...
      <div className="mt-8 rounded-[24px] border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-4 md:p-5">
        <label htmlFor="explorer-search" className="mb-3 block text-sm font-semibold text-[var(--color-text-primary)]">
          Rechercher un contenu
        </label>

        <div className="flex items-center gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-surface-2)] px-4 py-3 transition-colors focus-within:border-[var(--color-text-title)] focus-within:ring-2 focus-within:ring-[var(--color-text-title)]/20">
